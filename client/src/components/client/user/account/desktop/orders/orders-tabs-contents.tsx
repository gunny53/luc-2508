...
import { ReviewsModal } from '@/components/client/products/products-reviews-modal'
...
export const OrderTabContent = ({ currentTab, onTabChange }: Props) => {
  ...
  const { orders, loading, error, metadata, fetchAllOrders, fetchOrdersByStatus } = useOrder()
  ...
  return (
    <>
      {tabs.map((value) => (
        <TabsContent key={value} value={value} className="bg-white min-h-[70vh] p-4">
                          ...
                          <Button
                            ...
                            onClick={(e) => {
                              ...
                              setReviewProduct({
                                ...
                                productSlug: createProductSlug(firstItem.productName, firstItem.productId),
                                orderId: order.id
                              })
                            }}
                          >
                    ...
                    onClick={async () => {
                      if (orders.length < (metadata?.totalItems ?? 0)) {
                        if (currentTab === 'all') {
                          ...
                          await fetchOrdersByStatus(currentTab as OrderStatus, 1, metadata.totalItems)
                        }
                      }
