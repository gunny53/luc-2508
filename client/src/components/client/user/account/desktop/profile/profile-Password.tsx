...
import { usePasswordChangePassword, ChangePasswordResult } from './use-profile-change-password'
        ...
        <div className="space-y-4">
          {(
            [
              ...
              ['confirmNewPassword', 'confirmPassword', 'newPasswordPlaceholder']
            ] as const
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  <FormLabel>{t(`user.account.password.${labelKey}`)}</FormLabel>
                  <FormControl>
                    <div className="relative">
                      <Input
                        type={show[fieldName] ? 'text' : 'password'}
                        placeholder={t(`user.account.password.${placeholderKey}`)}
                        {...field}
                        className="pr-12"
                      />
                      <button
                        ...
                        tabIndex={-1}
                      >
                        {show[fieldName] ? <EyeOff size={18} /> : <Eye size={18} />}
                      </button>
                    </div>
                  </FormControl>
                  <FormMessage />
  ...
  return (
    <div className="bg-white rounded-lg p-6 space-y-4 border">
      <div className="flex justify-between items-center">
        <h2 className="font-semibold text-base text-[#121214]">{t('user.account.password.title')}</h2>
        <Button
          variant="ghost"
