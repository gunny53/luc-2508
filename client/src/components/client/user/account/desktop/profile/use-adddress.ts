...
export const useAddress = () => {
  ...
  const findMatchingAddress = async (addressDetail: any) => {
    try {
      ...
      const matchedProvince = provincesResult.data?.find(
        (p: any) => p.ProvinceName?.trim().toLowerCase() === addressDetail.province?.trim().toLowerCase()
      )
      ...
      const matchedDistrict = districtsResult.data?.find(
        (d: any) => d.DistrictName?.trim().toLowerCase() === addressDetail.district?.trim().toLowerCase()
      )
      ...
      const matchedWard = wardsResult.data?.find(
        (w: any) => w.WardName?.trim().toLowerCase() === addressDetail.ward?.trim().toLowerCase()
      )
  ...
  const prepareAddressForEdit = async (
    addressId: string
  ): Promise<{
  ...
  const handleSaveAddress = async (data: AddressFormValues, editingAddressId?: string, onSuccess?: () => void) => {
    const basePayload = {
  ...
  const formatFullAddress = (data: AddressFormValues) =>
    [data.detail, data.ward, data.district, data.province].filter(Boolean).join(', ')
