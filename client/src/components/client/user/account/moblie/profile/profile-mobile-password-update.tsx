...
import { usePasswordChangePassword } from '../../profile/use-profile-change-password'
...
export function ChangePasswordModal({ open, onOpenChange, name }: ChangePasswordModalProps) {
        ...
        <div className="h-[100vh] mx-auto w-full max-w-sm">
          <DrawerHeader>
            <div className="flex items-center gap-2">
              ...
              <DrawerTitle className="text-xl font-semibold">{t('user.account.password.title')}</DrawerTitle>
            </div>
            <button
          ...
          <div className="p-4 space-y-6">
            <div className="flex items-center gap-2 p-3 bg-blue-50 rounded-lg">
              ...
              <p className="text-xs text-blue-600">{t('user.account.password.subtitle')}</p>
            </div>
            ...
            <Form {...form}>
              ...
              >
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem className="space-y-1">
                      <FormLabel className="text-sm font-medium">{t('user.account.password.newPassword')}</FormLabel>
                      <FormControl>
                        <div className="relative">
                          <Input
                      ...
                      <p className="text-xs text-gray-500">{t('user.account.password.minLength')}</p>
                      <FormMessage className="text-xs" />
                    </FormItem>
                  )}
                />
                ...
                <div className="flex items-center space-x-2">
                  ...
                  />
                  <label htmlFor="revokeOtherSessions" className="text-sm text-gray-600 leading-none">
                    {t('user.account.password.revokeOtherSessions')}
                  </label>
                </div>

                <DrawerFooter className="border-t">
                  <Button type="submit" className="bg-red-600 text-white w-full" disabled={loading}>
                    {loading ? t('user.account.password.processing') : t('user.account.password.changePassword')}
                  </Button>
                </DrawerFooter>
              </form>
            </Form>
