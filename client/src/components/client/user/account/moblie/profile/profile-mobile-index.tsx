...
import { ChangePasswordModal } from './profile-mobile-password-update'
import { ProfileUpdateSheet } from './profile-mobile-update'
import { TwoFactorAuthModal } from './profile-mobile2-fa'
import { DeleteAccountModal } from './profile-mobile-delete-account'
import { useUserData } from '@/hooks/use-get-data-user-login'
import { usePasswordSecurity } from '../../profile/use-profile-2-fa'
import { useUserMobileHeader } from '@/providers/user-mobile-header-context'
    ...
    <div className="pb-8 space-y-4 text-sm text-black bg-white">
      <div className="flex items-center bg-white py-4">
        <div className="flex-shrink-0 flex items-center">
          <div className="h-10 w-10 rounded-full bg-green-100 flex items-center justify-center">
            <svg width="20" height="20" fill="currentColor" viewBox="0 0 24 24" className="text-green-600">
        ...
        <div className="flex-1 pr-4 px-1.5">
          <p className="text-sm font-semibold text-green-700">{t('user.account.profile.protected')}</p>
          <p className="text-sm text-gray-700 mt-1">{t('user.account.profile.protectionDescription')}</p>
        </div>
      </div>
      ...
      <div className="flex items-center justify-between border-b py-3">
        <div className="flex flex-col space-y-3">
          <div>
            <p className="text-black font-medium">{t('user.account.profile.username')}</p>
            <p className="text-gray-700 text-sm mt-1">@{name}</p>
          </div>

          <div>
            <p className="text-black font-medium">{t('user.account.profile.phone')}</p>
      ...
      <div className="flex items-center justify-between border-b py-3">
        <div className="flex flex-col">
          <p className="text-black font-medium">
            ...
            <span className="font-normal">{t(`user.account.profile.${user?.twoFactorEnabled ? 'on' : 'off'}`)}</span>
          </p>
          <p className="text-sm text-gray-600 mt-1">{t('user.account.profile.twoFactorDescription')}</p>
        </div>
        <Button
      ...
      <ChangePasswordModal open={isPasswordModalOpen} onOpenChange={setIsPasswordModalOpen} name={name} />
