...
function BannerSectionComponent() {
  ...
  return (
    <div className="relative h-[150px] md:h-[175px] overflow-hidden rounded-xl group perspective-[2000px] hover:shadow-2xl hover:shadow-black/20 transition-all duration-500">
      <motion.div initial={{ scale: 1 }} animate={{ scale: [1, 1.02, 1] }} className="absolute inset-0">
        <AnimatePresence initial={false} mode="popLayout" custom={direction} onExitComplete={() => setIsAutoplay(true)}>
          {banners.map(
            (banner, index) =>
              index === currentBanner && (
                <motion.div
