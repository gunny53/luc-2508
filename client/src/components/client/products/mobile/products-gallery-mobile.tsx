...
import { ChevronLeft, ShoppingCart, MoreVertical, PlayCircle } from 'lucide-react'
import { Button } from '@/components/ui/button'
import { MediaItem } from '../shared/product-transformers'
import styles from './products-gallery-mobile.module.css'
...
export default function ProductGalleryMobile({ media }: Props) {
  ...
  const handleTouchMove = (e: React.TouchEvent) => {
    ...
    wrapperRef.current.style.setProperty('--translate-x', `calc(${-currentIndex * 100}% + ${deltaX}px)`)
  }
    ...
    <div className="w-full aspect-square bg-white relative overflow-hidden">
      ...
      >
        {media.map((item, index) => (
          <div key={index} className="w-full h-full flex-shrink-0 flex items-center justify-center bg-black relative">
            {item.type === 'image' ? (
              <Image
                src={item.src}
                alt={`Image ${index + 1}`}
