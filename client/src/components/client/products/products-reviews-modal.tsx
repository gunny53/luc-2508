    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-3xl rounded-2xl p-4 shadow-xl">
        ...
        <div className="flex items-center gap-4 rounded-xl bg-gray-50 p-2">
          ...
          />
          <p className="flex-1 font-medium text-gray-800 text-lg leading-snug line-clamp-2">{product.productName}</p>
        </div>
          ...
          <div className="space-y-3">
            ...
            {files.length > 0 && (
              <div className="grid grid-cols-3 sm:grid-cols-4 gap-3">
                {files.map((file, idx) => (
                  <Card key={idx} className="relative group overflow-hidden rounded-xl border">
        ...
        <DialogFooter className="flex justify-end gap-3 pt-4">
          ...
          <Button
            onClick={async () => {
              await handleSubmit()
              if (product) {
                const slug = createProductSlug(product.productName, product.productId)
