...
const StarRating = ({ rating, size = 4 }: { rating: number; size?: number }) => (
  <div className="flex items-center">
    {[...Array(5)].map((_, i) => (
      <Star key={i} className={`h-${size} w-${size} ${i < rating ? 'text-red-500 fill-red-500' : 'text-gray-300'}`} />
    ))}
  </div>
)
            ...
            <div className="flex flex-wrap gap-2 mt-3">
              {review.medias.map((media) => (
                <div key={media.id} className="relative w-24 h-24 cursor-pointer">
                  <Image src={media.url} alt="Review media" fill className="rounded-md object-cover" />
                </div>
              ))}
            </div>
          )}
    ...
    <div className="bg-white p-6 rounded-lg shadow-sm font-sans">
      ...
      <div className="bg-neutral-50 p-4 rounded-lg flex flex-col md:flex-row md:items-center md:space-x-8 border">
        <div className="text-center text-red-600 md:pr-8 md:border-r pb-4 md:pb-0">
          <p className="text-lg">
            <span className="font-bold text-3xl">{summary.average.toFixed(1)}</span> /5
          </p>
