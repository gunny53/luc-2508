...
import { findMatchingSku, SelectedVariants, Sku } from '@/utils/product-utils'
...
export default function MobileCartItem({ item, selected, onToggle, onRemove }: MobileCartItemProps) {
  ...
  useEffect(() => {
    if (isPopoverOpen && productDetails) {
      ...
      setCurrentSku(findMatchingSku(initialVariants, skus, productDetails.variants))
    }
  ...
  const handleVariantSelect = (variantType: string, option: string) => {
    ...
    if (productDetails) {
      ...
      setCurrentSku(findMatchingSku(newSelected, skus, productDetails.variants))
    }
  }
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="mb-3">
              <Popover open={isPopoverOpen} onOpenChange={setIsPopoverOpen}>
                ...
                <PopoverContent className="w-80 p-0 rounded-xl shadow-lg border-0" align="start" sideOffset={4}>
                  <div className="p-4">
                    {isLoading ? (
                      ...
                      <div className="space-y-4">
                        ...
                        {productDetails.variants.map((v) => (
                          <div key={v.value} className="space-y-2">
                            <label className="text-sm font-medium text-gray-700">{v.value}</label>
                            <div className="flex flex-wrap gap-2">
                              {v.options.map((opt) => {
                                const isSelected = selectedVariants[v.value] === opt
                                return (
                                  <button
                                    key={opt}
                                    onClick={() => handleVariantSelect(v.value, opt)}
                                    className={cn(
                        ...
                        <div className="flex justify-end gap-2 pt-2">
                          ...
                          <Button
                            ...
                            disabled={!currentSku || currentSku.id === item.sku.id || isUpdating}
