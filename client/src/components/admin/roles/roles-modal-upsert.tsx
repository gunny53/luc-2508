...
interface RolesModalUpsertProps {
  ...
  mode: 'add' | 'edit'
  role?: Role | null
  onSubmit: (values: { name: string; description: string; isActive: boolean; permissionIds: string[] }) => Promise<void>
...
export default function RolesModalUpsert({
  ...
  const handleMasterSwitchChange = (subject: string, checked: boolean) => {
    const isRecordType = (
      data: Record<string, PermissionDetail[]> | PermissionDetail[]
  ...
  return (
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-4xl max-h-[90vh] flex flex-col">
        <DialogHeader>
          <DialogTitle>{mode === 'add' ? t('admin.roles.modal.title') : t('admin.roles.modalEdit.title')}</DialogTitle>
          <DialogDescription>
            {mode === 'add' ? t('admin.roles.modal.subtitle') : t('admin.roles.modalEdit.subtitle')}
          </DialogDescription>
        </DialogHeader>
        ...
        <form onSubmit={handleSubmit} className="flex-grow flex flex-col overflow-hidden">
            ...
            {mode === 'edit' && (
              <div className="space-y-2 pt-2">
                <div>
                  ...
                  <p className="text-sm text-muted-foreground mt-1">{t('admin.roles.modal.permissionsDescription')}</p>
                </div>
                    ...
                    <Accordion type="multiple" className="w-full">
                      {Object.entries(permissionsData || {}).map(([subject, items]) => {
                        ...
                        const selectedCount = items.filter((item: PermissionDetail) =>
                          selectedPermissionIds.has(item.id.toString())
                        ).length
          ...
          <DialogFooter className="pt-4 border-t mt-4 flex-shrink-0">
            ...
            <Button type="submit" disabled={loading}>
              {loading ? t('admin.roles.modal.processing') : t('admin.roles.modal.save')}
            </Button>
          </DialogFooter>
        </form>
      </DialogContent>
