...
export const CategoryColumns = (param: CategoryColumnsProps = {}): ColumnDef<CategoryTableData>[] => {
  ...
  return [
    ...
    },
    {
      ...
      header: ({ column }) => <DataTableColumnHeader column={column} title={t('parentCategory')} />,
      cell: ({ row }) => {
        const parentId = row.getValue('parentCategoryId')
        return <div>{parentId ? String(parentId) : t('noParent')}</div>
      },
      enableSorting: true,
      enableHiding: true
    },
      ...
      cell: ({ row }) => {
        ...
        return logo ? (
          <div className="h-10 w-10 relative">
            <img src={logo} alt={row.getValue('name')} className="h-full w-full object-contain rounded" />
          </div>
        ) : (
    ...
    },
    {
      id: 'actions',
      cell: ({ row }) => (
        <DataTableRowActions row={row} actions={getCategoryActions(row.original, onEdit, onDelete, t)} />
      ),
      enableSorting: false,
      enableHiding: false
    }
