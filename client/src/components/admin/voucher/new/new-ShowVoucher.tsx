...
export default function VoucherShowSettings({ formData, updateFormData, useCase, isEdit = false }: ShowVoucherProps) {
  ...
  const renderProductSelector = () => (
    <div className="w-full space-y-3">
      ...
      >
        <Plus className="-ml-1 mr-2 h-4 w-4" />English content normalized from the original source text.</Button>
        ...
        <div className="border border-gray-300 rounded-lg p-4 space-y-4 bg-gray-50">
          ...
          >
            ...
            {!loading && products.length === 0 && searchTerm && (
              <div className="text-center py-8 text-gray-600 text-sm">English content normalized from the original source text.</div>
            )}

            {!loading && !hasMore && products.length > 0 && (
              <div className="text-center py-2 text-gray-500 text-xs">English content normalized from the original source text.</div>
            )}
          </div>
        </div>
      )}
        ...
        <div className="space-y-3 border border-gray-300 rounded-lg p-4 bg-gray-50">
          <div className="flex items-center justify-between">
            <div className="text-sm text-gray-700 font-medium">
              English content normalized from the original source text. {selectedProducts.length} English content normalized from the original source text.
            </div>
            <Button
              variant="ghost"
              size="sm"
    ...
    if (useCase === VoucherUseCase.PRIVATE || useCase === VoucherUseCase.PRIVATE_ADMIN) {
      ...
      return (
        <div className="flex items-start space-x-6">
          <RequiredLabel htmlFor="display-type" className="mt-3 whitespace-nowrap">English content normalized from the original source text.</RequiredLabel>
            ...
            <div className={`flex items-center space-x-2 p-3 rounded-lg border ${isAdminPrivate ? 'bg-red-50 border-red-200' : 'bg-gray-100 border-gray-200'}`}>
              ...
              <Label htmlFor="display-private" className="font-normal text-gray-900">
                <div className="flex items-center gap-2">
                  <span>English content normalized from the original source text. {isAdminPrivate ? '(Admin)' : ''}</span>
                  {isAdminPrivate && (
                    <span className="px-2 py-0.5 bg-red-100 text-red-700 text-xs rounded-full">Platform</span>
                  )}
                </div>
                <p className="text-xs text-gray-600 mt-1">
                  English content normalized from the original source text. {isAdminPrivate ? 'English content normalized from the original source text.' : 'Shop'}.
                </p>
              </Label>
            </div>
          </RadioGroup>
      ...
      <div className="flex items-start space-x-6">
        ...
        >
          <div className="flex items-center space-x-2 rounded-lg hover:bg-gray-50 transition-colors p-1">
            ...
            <Label htmlFor="display-public" className="font-normal cursor-pointer text-gray-900">English content normalized from the original source text.</Label>
          </div>
          <div className="flex items-center space-x-2 rounded-lg hover:bg-gray-50 transition-colors p-1">
            <RadioGroupItem value="PRIVATE" id="display-private" />
            <Label htmlFor="display-private" className="font-normal cursor-pointer text-gray-900">English content normalized from the original source text.</Label>
          </div>
        </RadioGroup>
      </div>
    );
  ...
  const renderApplicableProducts = () => {
    switch(useCase) {
      case VoucherUseCase.SHOP:
        ...
        return (
          <div className="flex items-start space-x-6">
            <RequiredLabel className="mt-3 whitespace-nowrap">English content normalized from the original source text.</RequiredLabel>
            <RadioGroup value="ALL" className="w-full">
              <div className="flex items-center space-x-2 p-3 rounded-lg bg-gray-100 border border-gray-200">
                ...
                <Label htmlFor="apply-all" className="font-normal text-gray-900">English content normalized from the original source text.<p className="text-xs text-gray-600 mt-1">English content normalized from the original source text.</p>
                </Label>
              </div>
            </RadioGroup>
          </div>
          ...
          <div className="flex items-start space-x-6">
            ...
            <div className="w-full space-y-3">
              <div className="p-3 rounded-lg bg-red-50 border border-red-200">
                <div className="flex items-center gap-2">
                  <span className="text-sm text-red-800 font-medium">English content normalized from the original source text.</span>
                  <span className="px-2 py-0.5 bg-red-100 text-red-700 text-xs rounded-full">Platform</span>
                </div>
            ...
            <div className="w-full space-y-3">
              ...
              {formData.selectedShopUser && (
                <div className="p-3 rounded-lg bg-green-50 border border-green-200">
                  <div className="flex items-center gap-2">
                    <span className="text-sm text-green-800 font-medium">English content normalized from the original source text.</span>
                  </div>
          ...
          <div className="flex items-start space-x-6">
            ...
            <div className="w-full space-y-3">
              <div className="p-4 rounded-lg bg-blue-50 border border-blue-200">
                <div className="flex items-center gap-2 mb-2">
                  <span className="text-sm text-blue-800 font-medium">English content normalized from the original source text.</span>
      ...
      case VoucherUseCase.PRIVATE_ADMIN:
        ...
        return (
          <div className="flex items-start space-x-6">
            <RequiredLabel htmlFor="apply-type" className="mt-3 whitespace-nowrap">English content normalized from the original source text.</RequiredLabel>
            <div className="w-full space-y-3">
              ...
              >
                <div className="flex items-center space-x-2 p-3 rounded-lg border border-red-200 hover:bg-red-50 transition-colors">
                  ...
                  <Label htmlFor="apply-all-admin" className="font-normal cursor-pointer text-gray-900">English content normalized from the original source text.</Label>
                </div>
                <div className="p-3 rounded-lg border border-red-200 hover:bg-red-50 transition-colors">
                  <div className="flex items-center space-x-2 mb-3">
                    ...
                    <Label htmlFor="apply-specific-admin" className="font-normal cursor-pointer text-gray-900">English content normalized from the original source text.</Label>
                  </div>
          ...
          <div className="flex items-start space-x-6">
            ...
            >
              <div className="flex items-center space-x-2 p-3 rounded-lg border border-gray-200 hover:bg-gray-50 transition-colors">
                ...
                <Label htmlFor="apply-all" className="font-normal cursor-pointer text-gray-900">English content normalized from the original source text.</Label>
              </div>
              <div className="p-3 rounded-lg border border-gray-200 hover:bg-gray-50 transition-colors">
                <div className="flex items-center space-x-2">
                  ...
                  <Label htmlFor="apply-specific" className="font-normal cursor-pointer text-gray-900">English content normalized from the original source text.</Label>
                </div>
