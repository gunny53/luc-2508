...
import { ShoppingBag, Package2, Users, Star, Grid3X3, Building2, Tag, Crown } from 'lucide-react'
...
export default function VoucherFormCreate() {
  ...
  const getVoucherTypes = () => {
    if (isAdmin) {
      return adminVoucherTypes
    } else if (isSeller) {
      return sellerVoucherTypes
    }
    return []
  }
            ...
            return (
              <Card
                ...
                className={`overflow-hidden rounded-xs ${
                  isAdminVoucher ? 'border-red-200 bg-gradient-to-br from-red-50 to-red-100' : ''
                }`}
              >
                <CardContent className="p-0">
                  <div className="flex flex-col h-full">
                    <div className="p-4 flex items-start gap-3">
                      ...
                      >
                        <IconComponent className={`h-5 w-5 ${isAdminVoucher ? 'text-red-600' : 'text-primary'}`} />
                      </div>
                      <div className="flex flex-col">
                        <h3 className="font-semibold">{t(voucherType.nameKey)}</h3>
                        <p className="text-sm text-muted-foreground">{t(voucherType.descKey)}</p>
                        {isAdminVoucher && (
                    ...
                    <div className="mt-auto border-t p-3 px-6 text-right">
                      <Button
                        ...
                        className={`px-6 ${isAdminVoucher ? 'bg-red-600 hover:bg-red-700' : ''}`}
                      >
                        {t('create')}
                      </Button>
                    </div>
