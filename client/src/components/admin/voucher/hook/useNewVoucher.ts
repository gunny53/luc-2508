...
const getInitialFormData = (): VoucherFormState => {
  ...
  return {
    ...
    maxDiscountValue: null, // English content normalized from the original source text.
  };
};
...
interface UseNewVoucherProps {
  useCase: VoucherUseCase;
  owner: 'PLATFORM' | 'SHOP'; // English content normalized from the original source text.
  userData: any; // English content normalized from the original source text.
...
export function useNewVoucher({ useCase, owner, userData, onCreateSuccess }: UseNewVoucherProps): UseNewVoucherReturn {
  ...
  const getVoucherType = (uc: VoucherUseCase) => {
    ...
    if (uc === VoucherUseCase.PRIVATE) return VoucherType.SHOP; // English content normalized from the original source text.
  ...
  const submitVoucher = async (): Promise<void> => {
    ...
    const isPlatformVoucher = (useCase === VoucherUseCase.PLATFORM); // English content normalized from the original source text.
    ...
    try {
      ...
      const payload: CreateDiscountRequest = {
        name: formData.name,
        description: formData.description || formData.name, // English content normalized from the original source text.
        code: formData.code,
        ...
        shopId: null, // English content normalized from the original source text.
        isPlatform: isPlatformVoucher, // English content normalized from the original source text.
        voucherType: finalVoucherType, // English content normalized from the original source text.
      ...
      switch (useCase) {
        case VoucherUseCase.PLATFORM:
          ...
          payload.shopId = null;
          payload.discountApplyType = DiscountApplyType.ALL; // English content normalized from the original source text.
          break;
        ...
        case VoucherUseCase.SHOP_ADMIN:
          ...
          payload.isPlatform = false;
          payload.shopId = formData.selectedShopUser?.value || null; // English content normalized from the original source text.
        ...
        case VoucherUseCase.PRODUCT_ADMIN:
          ...
          payload.isPlatform = false;
          payload.shopId = null; // English content normalized from the original source text.
          // English content normalized from the original source text.
        ...
        case VoucherUseCase.PRIVATE_ADMIN:
          // English content normalized from the original source text.
          payload.voucherType = VoucherType.SHOP; // English content normalized from the original source text.
          payload.isPlatform = false;
          payload.shopId = null; // English content normalized from the original source text.
          payload.displayType = DisplayType.PRIVATE; // Force private
          payload.discountApplyType = formData.discountApplyType; // English content normalized from the original source text.
          break;

        default:
          // English content normalized from the original source text.
          payload.voucherType = finalVoucherType;
          payload.shopId = userData.id; // English content normalized from the original source text.
          payload.isPlatform = false;
          break;
      }
