...
interface VariantSettingsProps {
  variants: Variant[];
  skus: FormSku[]; // English content normalized from the original source text.
...
export function VariantSettingsIndex({
    ...
    updatedSkus.forEach((sku, index) => {
      try {
        ...
        updateSingleSku(indexToUpdate, {
          ...
          stock: sku.stock,
          image: sku.image || '',
          value: value, // English content normalized from the original source text.
          id: sku.id // English content normalized from the original source text.
        });
      } catch (error) {
  ...
  useEffect(() => {
    ...
    if (!initializedFromProps.current || !isInternalChange.current) {
      ...
      initializedFromProps.current = true; // English content normalized from the original source text.
    }
  ...
  return (
    <Card className="bg-white border-slate-200">
      <CardContent className="p-0">
        ...
        <SKUList
          options={options}
          initialSkus={skus} // English content normalized from the original source text.
