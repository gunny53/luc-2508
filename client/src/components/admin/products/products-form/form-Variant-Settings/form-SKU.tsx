...
import { Sku, generateApiVariantName } from '@/utils/variant-utils'
import type { OptionData } from './form-variant-input'
import { useSku, formatPrice } from './use-sku'
...
function SkuImageUploader({ skuId, imageUrl, onUploadComplete }: SkuImageUploaderProps) {
  ...
  useEffect(() => {
    if (uploadedUrls.length > 0) {
      const newUrl = typeof uploadedUrls[0] === 'string' ? uploadedUrls[0] : (uploadedUrls[0] as any).url || ''
...
export const SKUList = React.memo(function SKUList({ options, initialSkus, onUpdateSkus }: SKUListProps) {
  ...
  const skuHook = useSku({
    options,
    initialSkus,
    onUpdateSkus: React.useCallback(
      (updatedSkus) => {
  ...
  const { skus, groupedSkus, expandedGroups, handleSkuChange, toggleGroup, handleImageUpdate } = skuHook
          ...
          {Object.entries(groupedSkus).map(([groupKey, groupSkus]) => (
            <div key={groupKey} className="border-b border-slate-200 last:border-b-0">
              ...
              >
                ...
                <ChevronDown
                  className={`h-5 w-5 text-muted-foreground transition-transform ${expandedGroups[groupKey] ? 'rotate-180' : ''}`}
                />
              </button>
              {expandedGroups[groupKey] && (
                <div className="py-2">
                  {groupSkus.map((sku) => {
                    const variantName = sku.variantValues
                      .slice(1)
                      .map((v) => v.value)
                      .join(' / ')
                    return (
                      <div
                        key={sku.id}
                        className="px-4 grid grid-cols-12 gap-4 items-center py-2 border-t border-slate-200 first:border-t-0 hover:bg-slate-50 transition-colors"
                      >
                        <div className="col-span-1 flex items-center justify-center">
                          <SkuImageUploader skuId={sku.id} imageUrl={sku.image} onUploadComplete={handleImageUpdate} />
                        </div>
                        <div className="col-span-5 flex flex-col">
                          <span className="text-sm">{variantName}</span>
                          <span className="text-xs text-muted-foreground">
                            {generateApiVariantName(sku.variantValues)}
                          </span>
                        </div>
