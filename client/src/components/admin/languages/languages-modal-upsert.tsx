...
export default function LanguagesModalUpsert({ open, onClose, mode, language, onSubmit }: LanguagesModalUpsertProps) {
    ...
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>
            {mode === 'add' ? t('admin.languages.modal.subtitle') : t('admin.languages.modalEdit.subtitle')}
          </DialogDescription>
        </DialogHeader>
        <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div className="max-h-60 overflow-y-auto">
                  {filteredOptions.length === 0 ? (
                    <div className="p-2 text-sm text-muted-foreground">
                      {t('admin.languages.modal.noLanguagesFound')}
                    </div>
                  ) : (
          ...
          <DialogFooter>
            ...
            <Button type="submit" disabled={loading}>
              {loading
                ? mode === 'add'
                  ? t('admin.languages.modal.processing')
                  : t('admin.languages.modal.processing')
                : mode === 'add'
                  ? t('admin.languages.modal.save')
                  : t('admin.languages.modal.save')}
            </Button>
          </DialogFooter>
        </form>
