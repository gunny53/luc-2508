...
import { useDbSeller } from '../hooks/use-db-seller'
  ...
  return (
    <div className="space-y-6 p-6 h-screen bg-white">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold">B?ng ?i?u khi?n</h1>
          <p className="text-muted-foreground">B?ng ?i?u khi?n</p>
        </div>
        <Button
        ...
        >
          <RefreshCw
            className={`w-4 h-4 mr-2 ${sellerStats.isLoading || discountStats.isLoading ? 'animate-spin' : ''}`}
          />
          B?ng ?i?u khi?n
        </Button>
      </div>
          ...
          {orderStatsCards.map((stat, index) => (
            <Card key={index} className="p-6 hover:shadow-md transition-shadow">
              <div className="flex items-center justify-between">
                ...
                <div className={`p-3 rounded-full ${getColorClasses(stat.color)}`}>{stat.icon}</div>
              </div>
            </Card>
          ))}
        </div>
      ...
      <div className="grid gap-4 md:grid-cols-3">
        <Card className="p-6">
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-lg font-semibold">B?ng ?i?u khi?n</h2>
          ...
          <div className="space-y-3">
            ...
            <div className="flex justify-between items-center text-sm">
              ...
              <span className="font-medium text-orange-600">
                {(
                  sellerStats.pendingPayment +
                  sellerStats.pendingPackaging +
                  sellerStats.pendingDelivery
                ).toLocaleString()}
              </span>
            </div>
            <div className="text-xs text-muted-foreground p-2 bg-blue-50 rounded">
              📦 {sellerStats.totalOrders.toLocaleString()} B?ng ?i?u khi?n
            </div>
          </div>
        </Card>
        ...
        <Card className="p-6">
          ...
          <div className="space-y-3">
            <div className="flex justify-between items-center text-sm">
              <span>B?ng ?i?u khi?n</span>
              <span className="font-medium text-green-600">
                {discountStats.totalDiscounts > 0
                  ? ((discountStats.activeDiscounts / discountStats.totalDiscounts) * 100).toFixed(1)
                  : 0}
                %
              </span>
            </div>
            <div className="flex justify-between items-center text-sm">
              <span>B?ng ?i?u khi?n</span>
              <span className="font-medium text-red-600">{discountStats.expiredDiscounts.toLocaleString()}</span>
            </div>
            <div className="text-xs text-muted-foreground p-2 bg-purple-50 rounded">
              🎁 {discountStats.totalDiscounts.toLocaleString()} b?ng ?i?u khi?n
            </div>
          </div>
        </Card>
