      ...
      <Dialog open={showRecoveryCodesDialog} onOpenChange={setShowRecoveryCodesDialog}>
        <DialogContent className="max-w-lg bg-[#23272f] text-white rounded-xl p-0 overflow-hidden">
          <div className="px-8 pt-8 pb-2">
            <DialogTitle className="text-2xl font-bold mb-1">{t('admin.profileSettings.QrCode.title')}</DialogTitle>
          ...
          <div className="px-8 pb-8">
            <div className="mb-6">
              <div className="grid grid-cols-2 gap-2 mb-3">
                {recoveryCodes && recoveryCodes.length > 0 ? (
                  recoveryCodes.map((code, idx) => (
                    <div
                      key={idx}
                      className="bg-[#181a20] rounded px-3 py-2 font-mono text-base select-all text-center"
                    >
                      {code}
                    </div>
                  ))
                ) : (
                  <div className="col-span-2 text-center py-4">{t('admin.profileSettings.QrCode.nofound')}</div>
                )}
              </div>
              ...
              <div className="flex gap-3 mt-4">
                ...
                <Button onClick={downloadRecoveryCodes} className="bg-[#383c44] hover:bg-[#4c505a] text-white flex-1">
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    className="h-5 w-5 mr-2"
                    viewBox="0 0 24 24"
            ...
            <div className="flex items-center gap-4 mb-6">
              ...
              <div>
                ...
                <div className="text-gray-300 text-sm">
                  ...
                  <Link href="https://authy.com/" target="_blank" className="underline text-blue-400">
                    Authy
                  </Link>{' '}
                  {t('admin.profileSettings.QrCode.or')}{' '}
                  <Link
                    href="https://support.google.com/accounts/answer/1066447?hl=vi"
                    target="_blank"
                    className="underline text-blue-400"
                  >
                    Google Authenticator
                  </Link>{' '}
                  {t('admin.profileSettings.QrCode.or2')}
                </div>
              </div>
            </div>
            <div className="mb-6">
