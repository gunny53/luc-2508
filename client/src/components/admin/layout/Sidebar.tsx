...
export function Sidebar({ isOpen: externalOpen, onOpenChange, onCollapse }: SidebarProps) {
  ...
  const isActive = (href: string, item: SidebarItem) => {
    ...
    if (href === '/admin/order' && pathname.startsWith('/admin/order')) return true
    return false
  }
        ...
        if (item.isTitle) {
          return (
            <div key={item.href} className="mb-2">
              ...
              >
                {item.icon && <span className="mr-1 text-black">{item.icon}</span>}
        ...
        return (
          <div key={item.href} className="mb-2">
            <div className="flex items-center justify-between">
              <div className="text-sm font-semibold text-black tracking-wide mb-1 mt-2">{item.title}</div>
            </div>
            {hasSub && (
            ...
            {idx < settingsSidebarConfig.length - 1 && <div className="border-t border-dotted border-gray-200 my-3" />}
          </div>
        )
      })}
    </div>
        ...
        >
          {hasSubItems ? (
            <div className="flex items-center gap-3 flex-1">
              {level === 0 && item.icon && React.isValidElement(item.icon) && item.icon.type
                ? React.createElement(item.icon.type, {
                    className: cn('w-5 h-5', isItemActive ? 'text-primary' : 'text-black')
                  })
                : null}
              <span
                className={cn(
  ...
  return (
    <>
      {isMobile && open && <div className="fixed inset-0 bg-black/40 z-40" onClick={() => setOpen(false)} />}
