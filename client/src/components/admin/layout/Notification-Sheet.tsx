    ...
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent side="right" className="max-w-lg w-full p-0 flex flex-col rounded-md ">
        <div className="flex items-center justify-between px-8 pt-5 pb-4 border-b">
          <SheetTitle className="text-lg font-semibold">{t('admin.notifications.title')}</SheetTitle>
        ...
        <div className="flex-1 flex flex-col items-center justify-center text-center px-22">
          ...
          <div className="font-semibold text-md mb-1">{t('admin.notifications.emptyTitle')}</div>
          <div className="text-muted-foreground text-[12px] mb-12">{t('admin.notifications.emptyDesc')}</div>
        </div>
      </SheetContent>
    </Sheet>
  )
