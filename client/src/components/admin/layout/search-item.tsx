...
import { useSidebarConfig, SidebarItem } from '@/constants/sidebar-config'
    ...
    <ul>
      {items.map((item) => (
        <React.Fragment key={item.href}>
          ...
          >
            {item.icon && level === 0 && React.isValidElement(item.icon) && item.icon.type
          ...
          {item.subItems && item.subItems.length > 0 && expanded.includes(item.href) && (
            <li>{renderMenu(item.subItems, level + 1)}</li>
          )}
        </React.Fragment>
      ))}
    </ul>
