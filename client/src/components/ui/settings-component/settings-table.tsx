          ...
          {columns.map((column, index) => (
            ...
            >
              <div className="text-gray-600 text-sm font-medium flex-shrink-0 flex items-center gap-2">
                <div className="w-4 h-4 text-gray-400 flex items-center justify-center">{column.startIcon}</div>
                {column.label}
              </div>
              <div className="break-words text-gray-900 text-sm flex items-center justify-between">
                <div>{column.value}</div>
                <div className="w-4 h-4 text-gray-700 flex items-center justify-center">{column.endIcon}</div>
              </div>
            </div>
          ))}
          {children}
