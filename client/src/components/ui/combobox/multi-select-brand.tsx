...
import { useCbbBrand } from '@/hooks/combobox/use-cbb-brand'
...
export function MultiSelectBrand({
  ...
  const filteredBrands = brands.filter((brand) => brand.label.toLowerCase().includes(searchTerm.toLowerCase()))
    ...
    <div className={cn('w-full', className)}>
              ...
              {selectedBrands.length === 0 ? (
                ...
                <>
                  {selectedBrands.slice(0, 3).map((brand) => (
                    <Badge key={brand.value} variant="secondary" className="text-xs flex items-center gap-1">
                      {brand.image && (
                        <img src={brand.image} alt={brand.label} className="w-3 h-3 rounded-full object-cover" />
                      )}
                      {brand.label}
                      <X
                ...
                <>
                  {selectedBrands.length > 0 && (
                    <div className="p-2 border-b">
                      ...
                      >
                        Clear selected brands ({selectedBrands.length})
                      </Button>
                    </div>
                  )}
                  <CommandGroup>
      ...
      {selectedBrands.length > 0 && (
        <div className="mt-2 space-y-2">
          <div className="text-sm text-muted-foreground">{selectedBrands.length} selected brands.</div>
          <div className="flex flex-wrap gap-1">
            {selectedBrands.map((brand) => (
              <Badge key={brand.value} variant="outline" className="text-xs flex items-center gap-1">
                {brand.image && (
                  <img src={brand.image} alt={brand.label} className="w-3 h-3 rounded-full object-cover" />
                )}
                {brand.label}
                <X className="h-3 w-3 cursor-pointer hover:text-red-500" onClick={() => handleRemove(brand.value)} />
              </Badge>
            ))}
          </div>
        </div>
