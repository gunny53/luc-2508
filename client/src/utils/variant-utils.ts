import type { OptionData } from '@/components/admin/products/products-form/form-variant-settings/form-variant-input'
...
export const generateSKUs = (options: OptionData[]): Sku[] => {
  ...
  const activeOptions = options.filter(
    (opt) => opt.name.trim() !== '' && opt.values.length > 0 && opt.values.some((v) => v.trim() !== '')
  )
