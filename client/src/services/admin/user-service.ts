...
export const userService = {
  ...
  async getById(id: string): Promise<{ data: User }> {
    const response = await privateAxios.get(API_ENDPOINTS.USERS.GETBYID.replace(':id', String(id)))
    return response.data
  },
  async create(data: UserCreateRequest): Promise<UserCreateResponse> {
    const response = await privateAxios.post(API_ENDPOINTS.USERS.POST, data)
    return response.data
  },
  async update(id: string, data: UserUpdateRequest): Promise<UserUpdateResponse> {
    const response = await privateAxios.put(API_ENDPOINTS.USERS.UPDATE.replace(':id', String(id)), data)
    return response.data
  },
  async delete(id: string): Promise<UserDeleteResponse> {
    const response = await privateAxios.delete(API_ENDPOINTS.USERS.DELETE_BY_ID.replace(':id', String(id)))
    return response.data
  }
}
